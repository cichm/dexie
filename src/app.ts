...
async function haveSomeFun() {
    ...
    await db.transaction('rw', db.contacts, db.emails, db.phones, async function () {
        ...
        // ... and another one...
        let JohnId = await db.contacts.add(new Contact('John', 'Skeet'));
        // Populate some emails and phone numbers for the contact
        db.emails.add({ contactId: JohnId, type: 'home', email: 'John@Skeet.se' });
        db.phones.add({ contactId: JohnId, type: 'work', phone: '88888888' });
    });

    //
    // For fun - add a phone number to John
    //
    ...
    console.log("Playing a little: adding another phone entry for John Skeet...");
    let John = await db.contacts.orderBy('lastName').last();
    console.log(`Found contact: ${John.firstName} ${John.lastName} (id: ${John.id})`);
    
    // To add another phone number to John, the straight forward way would be this:
    await db.phones.add({contactId: John.id, type: "custom", phone: "+46 7777777"});

    // But now let's do that same thing by manipulating navigation property instead:
    // Load emails and phones navigation properties
    await John.loadNavigationProperties();
    
    // Now, just push another phone number to John.phones navigation property:
    John.phones.push({
        contactId: John.id,
        type: 'custom',
        phone: '112'
